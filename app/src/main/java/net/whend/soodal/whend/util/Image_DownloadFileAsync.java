...
//		mDlg.show();
	...
	protected String doInBackground(String... params) {
		...
		try {
			...
			Log.d("fileexist", path + " : " + String.valueOf(new File("/sdcard"+path).exists()));
			//Log.d("isItFalse?",String.valueOf(path.compareTo("/and_null")));
						
			
			if(path.compareTo("/and_null")!=0 && path.compareTo("/and_")!=0 && !new File("/sdcard"+path).exists()){		//파일 존재하면 밑애꺼 안함.
